...
> project (plug-in) are documented in [`src/Gemini.md`](./src/Gemini.md). Be sure to read it
...
> Calculates / DataControls …) follow [`src/Gemini.md`](./src/Gemini.md).
...
- **Translation keys must be added to all three regional files.** (Details: `src/Gemini.md`)
...
| (old) using pyscript | `src/utils_pyscript.ts` | Fully commented out. To re-enable, see section 7 below / `src/Gemini.md` |

For detailed call examples, see the "API Request Guide" in [`src/Gemini.md`](./src/Gemini.md).
...
**To re-enable**: uncomment the three places above. (For detailed usage, see `src/Gemini.md`)
