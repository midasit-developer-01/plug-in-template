...
> [`src/Gemini.md`](./src/Gemini.md) 에 정리되어 있습니다. 코드 작업 전 반드시 함께 읽으세요.
...
> [`src/Gemini.md`](./src/Gemini.md) 를 따릅니다.
...
- **번역 키는 세 지역 파일에 모두 추가**해야 합니다. (상세: `src/Gemini.md`)
...
| (구) pyscript 사용 | `src/utils_pyscript.ts` | 전체 주석 처리됨. 다시 켜는 절차는 아래 7항 / `src/Gemini.md` 참고 |

상세한 호출 예시는 [`src/Gemini.md`](./src/Gemini.md) 의 "API 요청 가이드"를 참고하세요.
...
**다시 켜려면**: 위 세 곳의 주석을 해제하면 됩니다. (자세한 사용법은 `src/Gemini.md`)
